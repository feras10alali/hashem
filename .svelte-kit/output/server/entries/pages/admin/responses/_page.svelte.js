...
function _page($$payload, $$props) {
  ...
  $$payload.out += `<!--]--> <h1 class="text-3xl font-bold text-center my-8">جميع الردود</h1> `;
  if (store_get($$store_subs ??= {}, "$responses", responses).length === 0) {
    $$payload.out += "<!--[-->";
    $$payload.out += `<p class="text-center text-gray-500">لا توجد ردود بعد.</p>`;
  } else {
    $$payload.out += "<!--[!-->";
    const each_array = ensure_array_like(store_get($$store_subs ??= {}, "$responses", responses));
    ...
    for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
      ...
      $$payload.out += `<!--]--> <div class="mt-4 flex gap-3"><button class="bg-green-600 text-white px-4 py-2 rounded-md shadow hover:bg-green-700 transition">قبول</button> <button class="bg-red-600 text-white px-4 py-2 rounded-md shadow hover:bg-red-700 transition">رفض</button></div></li>`;
    }
    $$payload.out += `<!--]--></ul>`;
  }
  $$payload.out += `<!--]--> `;
